import { drawerStyles } from "./styles.mjs";
...
*/
var CopilotKitThreadsDrawer = class extends LitElement {
	constructor(..._args) {
		super(..._args);
		this.label = "Threads";
	...
	*/
	...
	render() {
      ...
      ${this._isMobileModalOpen() ? html`<button
            class="backdrop"
            part="backdrop"
      ...
      >
        ${desktopCollapsed ? nothing : html`${this._renderHeader()} ${this._renderBody()}
	...
	*/
	...
	_renderHeader() {
      ...
      >
        ...
        ></slot>
        ${showCollapseToggle ? html`<button
              class="icon-btn"
	...
	*/
	_renderSectionHeading() {
		if (!this.licensed || hasErrorMessage(this.error) && this.threads.length === 0) return nothing;
		return html`
      <div class="section-heading" part="section-heading">
        ...
        >
          ${iconFunnel}
          ${this._filter !== "active" ? html`
                  <span class="filter-dot" part="filter-indicator"></span>
                ` : nothing}
        </button>
        ${this._filterOpen ? html`<div
	...
	*/
	...
	_renderList() {
      ...
      <ul
        ...
        aria-label=${this.label}
      >
        ${repeat(visible, (thread) => thread.id, (thread, index) => this._renderRow(thread, index, visible.length))}
      </ul>
	...
	_renderRow(thread, index = 0, total = 1) {
      ...
      >
        ${slotName !== null ? html`<slot name=${slotName}>${nameSpan}</slot>` : nameSpan}
        <button
          class="row-menu"
          part="row-menu"
	...
	_renderFetchMore() {
		if (hasErrorMessage(this.fetchMoreError)) return html`
      ...
      `;
		if (this.fetchingMore) return html`
...
//# sourceMappingURL=copilotkit-threads-drawer.mjs.map